...
{
	static_assert(!std::is_same<Types, Types>::value,
		...
		if constexpr (is_move_constructible_v<T>)
		{
			...
			if  constexpr (is_default_constructible_v<T>)
			{
				...
				if (!is_equal(x, z) && is_equal(x, y))
					cout << "\tRemark: move contsructor is actually copy constructor.\n";
			}
			else
			{
				...
				if (is_equal(x, y))
					cout << "\tRemark: move constructor is actuall copy-constructor.\n";
			}
		}
		else
		{
...
void move_assignment_sanity(T && x)
{
	if constexpr (is_move_constructible_v<T> || is_copy_constructible_v<T>)
	{
		if constexpr (is_move_constructible_v<T>)
		{
			...
			{
				cout << "\tRemark: is move - assignable but not default - constructible.\n";
			}
		}
		else
		{
			...
			{
				T y = x;
				cout << typeid(T).name() << " is only copy-assignable. Copied- form object is "
					<< (is_equal(x, y) ? "equal to" : "different from") << " target object.\n";
			}
		}
	}
